...
class BaseDashboardPlaceholder(object):
    ...
    def multiple_plugins_placeholder(self, dashboard_entries=None):
        ignore_list = []
        pos = []
        pos_repeated = []
        need_switch = {}
        ...
        for index in pos:
            ...
            for value in pos:
                if index == value:
                    count += 1
            if count > 1 and index not in pos_repeated:
                pos_repeated.append(index)

        for index in pos_repeated:
            need_switch[index] = []
            for entry in dashboard_entries:
                if index == entry.position:
                    need_switch[index].append(entry)
                    ignore_list.append(entry)

        return need_switch, ignore_list
    ...
    def render_for_view(self):
        ...
        """
        try:
            need_switch, ignore_list = self.multiple_plugins_placeholder(self.dashboard_entries)
            context = {
                'ignore': ignore_list,
                'widget_switcher': need_switch,
...
class BaseDashboardPlugin(object):
    ...
    def _update_plugin_data(self, dashboard_entry):
        ...
        try:
            ...
            print dashboard_entry.plugin_uid+" updated..."
