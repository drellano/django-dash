...
urlpatterns = patterns('dash.views',
    ...
    url(r'^entry/paste/(?P<placeholder_uid>[\w_]+)/(?P<workspace>[\w_\-]+)/pos/(?P<position>\d+)/$', \
        view='paste_dashboard_entry', name='dash.paste_dashboard_entry'),
    url(r'^entry/paste/(?P<placeholder_uid>[\w_]+)/pos/(?P<position>\d+)/$', view='paste_dashboard_entry', \
        name='dash.paste_dashboard_entry'),

    # Cut dashboard entry
    url(r'^entry/cut/(?P<entry_id>\d+)/$', view='cut_dashboard_entry', \
        name='dash.cut_dashboard_entry'),

    # Copy dashboard entry
    url(r'^entry/copy/(?P<entry_id>\d+)/$', view='copy_dashboard_entry', \
    ...
    # Edit dashboard widget data.
    url(r'^edit/widget/(?P<layout_uid>[\w_]+)/(?P<workspace>[\w_\-]+)/$', view='edit_widget_data', \
    ...
    url(r'^entry/delete/(?P<entry_id>\d+)/$', view='delete_dashboard_entry', \
        name='dash.delete_dashboard_entry'),

    # ********************** Edit dashboard
    # Edit dashboard.
    url(r'^edit/(?P<workspace>[\w_\-]+)/$', view='edit_dashboard', \
    ...
    url(r'^plugin-widgets/(?P<placeholder_uid>[\w_]+)/(?P<workspace>[\w_\-]+)/$', \
        view='plugin_widgets', \
        name='dash.plugin_widgets'),
    url(r'^plugin-widgets/(?P<placeholder_uid>[\w_]+)/$', view='plugin_widgets', \
        name='dash.widgets'),

    # ********************** Dashboard workspace
    # List workspaces.
    url(r'^workspaces/(?P<workspace>[\w_\-]+)/$', view='dashboard_workspaces', \
        name='dash.dashboard_workspaces'),
    url(r'^workspaces/$', view='dashboard_workspaces', \
        name='dash.dashboard_workspaces'),

    # Create dashboard workspace.
    url(r'^workspace/create/$', view='create_dashboard_workspace', \
        name='dash.create_dashboard_workspace'),

    # Edit dashboard workspace.
    url(r'^workspace/edit/(?P<workspace_id>\d+)/$', \
        view='edit_dashboard_workspace', name='dash.edit_dashboard_workspace'),
    ...
    url(r'^workspace/(?P<workspace>[\w_\-]+)/$', view='dashboard', \
        name='dash.dashboard'),

    # Edit dashboard settings.
    url(r'^settings/edit/$', view='edit_dashboard_settings', \
